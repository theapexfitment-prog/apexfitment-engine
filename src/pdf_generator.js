...
function generateQuotePdf(quoteData, shop, outputStream) {
  ...
  // ── LEGAL DISCLAIMER ──────────────────────────────────────────────────────
  const DY = 660;
  doc.moveTo(L, DY).lineTo(R, DY).strokeColor(C.border).lineWidth(0.3).stroke();
  const disclaimerText =
    'ENGINEERING DISCLAIMER: This document constitutes a Theoretical Geometry Analysis generated by ApexFitment Engine v1.0. ' +
    'All compatibility calculations incorporate a ±3mm fabrication tolerance variable and represent an elimination of approximately 95% of fitment error ' +
    'based on declared build configuration variables. The remaining tolerance window requires artisanal adjustment at the discretion of the installing technician. ' +
    'ApexFitment LLC assumes no liability for dimensional variances resulting from undeclared modifications, manufacturing tolerances of individual components, ' +
    'or installation deviations. This analysis is a decision-support tool, not a guarantee of assembly. ' +
    'Verify all critical clearances before final installation.';
  doc.font('Helvetica-Oblique').fontSize(7.5).fillColor(C.gray)
     .text(disclaimerText, L, DY + 6, { width: W, lineBreak: true, lineGap: 1.5 });
